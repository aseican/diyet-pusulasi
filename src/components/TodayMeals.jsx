...
const TodayMeals = ({ meals, deleteMeal, selectedDate }) => {
  const handleDelete = (mealId) => deleteMeal(mealId);

  // ✅ Eğer parent tüm günleri gönderirse bile burada bugüne filtrele
  const day = selectedDate || localYMD();

  const todayMeals = useMemo(() => {
    const arr = Array.isArray(meals) ? meals : [];
    return arr.filter((m) => {
      // m.date yoksa dokunma (parent zaten filtrelemeli) ama varsa günü eşitle
      if (!m?.date) return true;
      // date "YYYY-MM-DD" veya timestamp gelebilir
  ...
  const mealsByType = Object.keys(mealTypeConfig).map((typeName) => {
    ...
    const items = todayMeals.filter((meal) => meal.meal_type === typeName);
    return {
      name: typeName,
      ...config,
      items,
      totalCalories: items.reduce((sum, item) => sum + n(item.calories), 0),
    };
  });

  const totalCalories = todayMeals.reduce((sum, meal) => sum + n(meal.calories), 0);
            ...
            >
              ...
              <ul className="space-y-2">
                {mealType.items.map((item) => (
                  <li
                    key={item.id}
                    className="flex justify-between items-center p-2 rounded-lg hover:bg-gray-50 group"
                  >
                    <div className="flex-grow">
                      <p className="font-medium text-gray-800">{item.food_name}</p>
                      <p className="text-sm text-gray-500">
                        {Math.round(n(item.calories))} kcal • {n(item.quantity)} {item.unit}
                      </p>
                      <div className="flex gap-x-3 text-xs text-gray-400 mt-1">
                        <span>P: {n(item.protein).toFixed(1)}g</span>
                        <span>K: {n(item.carbs).toFixed(1)}g</span>
                        <span>Y: {n(item.fat).toFixed(1)}g</span>
                      </div>
                    </div>
                    ...
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="text-gray-400 hover:text-red-600 hover:bg-red-50 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
                        >
